...
def main():
    ...
    for thread_blas_count in sorted(thread_blas_counts):
        ...
        if os.path.exists(file_path):
            ...
            for image_id, data in results.items():
                ...
                thread_blas_results[image_id].append(data)
        else:
            print(f"File {file_path} non trovato.")
    ...
    blas_times_blas = [best_configs_blas[img_id]['total_time'] for img_id in image_ids]
    ...
    best_blas_bars = ax.bar(x, best_times_blas, width, label='SLIC + Matrix Mean Shift (OpenBLAS)')
