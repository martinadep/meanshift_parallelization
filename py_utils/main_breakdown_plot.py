...
kernels = sorted(list(detected_kernels)) if detected_kernels else kernels
...
for i, bw in enumerate(bandwidths):
    offset = (i - len(bandwidths)//2) * bar_width  # Offset per posizionare le colonne della bandwidth
    bottom = np.zeros(len(kernels))
    for label in timing_labels:
        heights = [data[kernel][bw][label] for kernel in kernels]
        ax.bar(x + offset + x_offset, heights, bar_width, label=f"{label} (bw={bw})" if i == 0 else "", 
               bottom=bottom, color=colors[label])
        bottom += heights

    # Aggiungi etichette delle bandwidth sotto ogni colonna
    for j, kernel in enumerate(kernels):
        ax.text(x[j] + offset + x_offset, -0.5, f"bw={bw}", ha="center", va="top", fontsize=10, color="black")
...
# Salva il grafico
plt.tight_layout()
plt.savefig("./test/combined_execution_time_sqrd.png")
plt.show()