                      ...
                      unsigned int *cluster_count)
{
    ...
    for (int i = 0; i < dataset_size; i++){
        shift_point_until_convergence_sqrd(&dataset[i], &shifted_dataset[i],
    ...
    for (int i = 0; i < dataset_size; i++){
        assign_clusters_sqrd(&shifted_dataset[i], cluster_modes, cluster_count);
    }
}
                                   ...
                                   unsigned int bandwidth, T (*kernel_func)(T, unsigned int))
{
    ...
    while (!stop_moving)
    {
        shift_single_point_sqrd(&prev_point, &next_point, dataset, dataset_size, bandwidth, kernel_func);

        T shift_distance = sqrd_euclidean_distance(&prev_point, &next_point);
